...
function init() {
    ...
    renderPdfs();
    renderNotes();
    updateDashboard(); // Run once after initial renders
}
...
function updateDashboard() {
    if (!document.getElementById('dash-cgpa')) return; // Safety check

    // Overall CGPA from Semesters
    let totalSgpa = 0;
    semestersList.forEach(sem => totalSgpa += sem.sgpa);
    const dashCgpaEl = document.getElementById('dash-cgpa');
    if (semestersList.length > 0) {
        dashCgpaEl.textContent = (totalSgpa / semestersList.length).toFixed(2);
        dashCgpaEl.style.color = 'var(--text-primary)';
    } else {
        dashCgpaEl.textContent = 'N/A';
        dashCgpaEl.style.color = 'var(--text-secondary)';
    }

    // Number of Subjects
    document.getElementById('dash-subjects').textContent = attSubjectsList.length;

    // Average Attendance
    let overTotal = 0;
    let overAtt = 0;
    attSubjectsList.forEach(sub => {
        overTotal += sub.total;
        overAtt += sub.attended;
    });

    const dashAttEl = document.getElementById('dash-attendance');
    const dashAttStatusEl = document.getElementById('dash-att-status');

    if (overTotal > 0) {
        const pct = (overAtt / overTotal) * 100;
        dashAttEl.textContent = pct.toFixed(1) + '%';
        if (pct >= 75) {
            dashAttEl.className = 'text-safe';
            dashAttStatusEl.className = 'text-safe';
            dashAttStatusEl.textContent = 'Safe status';
        } else {
            dashAttEl.className = 'text-danger';
            dashAttStatusEl.className = 'text-danger';
            dashAttStatusEl.textContent = 'Action needed';
        }
    } else {
        dashAttEl.textContent = '0%';
        dashAttEl.className = '';
        dashAttEl.style.color = 'var(--text-primary)';
        dashAttStatusEl.textContent = '-';
        dashAttStatusEl.className = '';
        dashAttStatusEl.style.color = 'var(--text-secondary)';
    }

    // Saved PDFs Count
    document.getElementById('dash-pdfs').textContent = pdfsList.length;

    // Latest Note widget
    const latestNoteEl = document.getElementById('dash-latest-note');
    if (notesList.length > 0) {
        latestNoteEl.textContent = `"${notesList[0].text}"`;
        latestNoteEl.style.fontStyle = 'italic';
    } else {
        latestNoteEl.textContent = 'No notes added yet.';
        latestNoteEl.style.fontStyle = 'normal';
    }
}
